...
export class AdminpageComponent implements OnInit {
  ...
  ngOnInit(): void {
      if(localStorage["user"] != 'olivera' && localStorage["user"] != 'abijelic' && localStorage["user"] != 'andrija') {
        //this.router.navigate(['unauthorized']);
        this.router.navigate(['admin']);
        return;
      }
      this.httpC.get(environment.authURL + 'getLogged').subscribe(data => {
        //logged
        this.user = data;
        if(this.user.email != 'olivera' && this.user.email != 'abijelic' && this.user.email != 'andrija') {
          //this.router.navigate(['unauthorized']);
          this.router.navigate(['admin']); 
         return;
        }
  ...
  generateFile() : Observable<any> {
    return this.httpC.get(environment.pdvURL + "generateExcel/" + this.year.value,  { responseType: 'blob'});
  }

  generateFileDrugo() : Observable<any> {
    return this.httpC.get(environment.pdvURL + "generateExcelTroskovi/" + this.year.value,  { responseType: 'blob'});
  }

  generateFilePoreskiPodaci(type) : Observable<any> {
    return this.httpC.get(environment.pdvURL + "generatePoreskiGubici/" + type + "/" + this.year.value, { responseType: 'blob'});
  }
